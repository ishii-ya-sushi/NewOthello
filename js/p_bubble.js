...
function p_bubble(x, y, attacker) {
  ...
  const effectName = arreyJson[1];

  p_e_bubble(x, y, attacker, effectName);
}

function p_e_bubble(x, y, attacker, effectName) {
  // リスナーを全部削除してタイマを止める
  ...
  // 定期的に呼ばれる関数を登録
  createjs.Ticker.addEventListener("tick", handleTick_bubble);
}

function handleTick_bubble() {
