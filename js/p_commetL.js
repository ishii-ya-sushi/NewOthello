...
function p_commetL(x, y, attacker) {
  ...
  const effectName = arreyJson[4];

  p_e_commetL(x, y, attacker, effectName);
}

function p_e_commetL(x, y, attacker, effectName) {
  // リスナーを全部削除してタイマを止める
  ...
  // 定期的に呼ばれる関数を登録
  createjs.Ticker.addEventListener("tick", handleTick_commetL);
}

function handleTick_commetL() {
  ...
  // particleSystem.startScale = 0   ,            // 開始時のスケール          (0.00-1.00)
  if (0 < count && count < 100) {
    particleSystem.startScale += 0.05;
  } else if (0 < count && count < 400)  {
    particleSystem.startScale = 0.9;
  }
  
  
  
  // particleSystem.startScaleVariance = 0.33,    // 開始時のスケールのばらつき(0.00-4.00)
  // particleSystem.finishScale = 0,              // 終了時のスケール          (0.00-1.00)
  // particleSystem.finishScaleVariance = 0,      // 終了時のスケールのばらつき(0.00-4.00)
  // particleSystem.lifeSpan = "120",              // ライフ(フレーム数)           (0-500)
  if (0 < count && count < 200) {
    particleSystem.lifeSpan = 250;
  } else if (200 < count && count < 1000)  {
    particleSystem.lifeSpan = 30;
  }else if (600 < count && count < 1000)  {
    particleSystem.lifeSpan = 40;
  }
