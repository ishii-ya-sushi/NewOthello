...
function p_start(x, y, attacker) {
  ...
  const effectName = arreyJson[0];

  p_e_start(x, y, attacker, effectName);
}

function p_e_start(x, y, attacker, effectName) {
  // リスナーを全部削除してタイマを止める
  ...
  // 定期的に呼ばれる関数を登録
  createjs.Ticker.addEventListener("tick", handleTick_start);
}

function handleTick_start() {
