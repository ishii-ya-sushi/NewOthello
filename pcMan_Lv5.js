// ---STEP.99   pcMan（黒）の番------------------------------------------
// ---STEP.99.1 置ける場所を探す。あるならクリック。ないならパス
//             ルール上、石を置けない所はクリックできない。同じ場所には２度クリックできない。

function pcManSide() {
    console.log("黒(先手、pcMan)のだあだだだだっだだだだだだあっだっだだだだっだあだあだあｔ");
    ...
    if (ArrayPass.length == 0) {
        ...
        console.log("置くところがあるのでpcManClick()へ");

        // greyMatter()にいく
        greyMatter()

        // setTimeout(pcManClick, 1700);
        setTimeout(pcManClick, 2000);
    }
}



// ---STEP.99.2 pcMan（先手、黒）が石を置く

function pcManClick() {
    console.log("pcMan ・ ・ ・ ・ ・ ・ 考 ・ ・ ・ ・ ・ ・ え ・ ・ ・ ・ ・ ・ 中");
    ...
    pcManalgorithm3(); // okeru2D[]の"strategyPoint"を降順でソートする

    pcManalgorithm4(); // okeru2D[]のstrategyPointをcumulativeSum2D[]に累積していく

    pcManalgorithm5(); // どの座標(x,y)に白石を置くか決定してPC側のクリックをする
